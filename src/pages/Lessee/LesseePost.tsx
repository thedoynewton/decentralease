...
export default function LesseePost() {
  ...
  const [locationInputValue, setLocationInputValue] = useState(formData.location); // Separate state for input to allow debouncing
  ...
  const handleSubmit = useCallback(async (e: FormEvent) => {
    ...
    if (!formData.title || !formData.description || !formData.category || !formData.location ||
      !formData.pickupDate || !formData.returnDate || formData.images.length === 0) {
  ...
  useEffect(() => {
    const fetchLocationSuggestions = async () => {
      ...
      try {
        const response = await fetch(`/api/places-autocomplete?input=${encodeURIComponent(debouncedLocationInput)}`);
        ...
        setLocationSearchError(`Network error: ${error.message || 'Could not fetch suggestions.'}`);
        setLocationSuggestions([]);
      } finally {
        setIsSearchingLocation(false);
      }
  ...
  return (
    <div className={styles.container}>
      <Head>
        ...
        <meta name="description" content="Create a new post for an asset you want to rent out." />
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={styles.formGroup}>
          ...
          <input
            ...
            id="location"
            name="location"
            value={locationInputValue} // Use separate state for input field
            onChange={handleLocationInputChange}
            required // Make sure location is required
            className={styles.input}
            placeholder="Search for a city or address"
          />
          {isSearchingLocation && <p className={styles.suggestionStatus}>Searching...</p>}
          {locationSearchError && <p className={`${styles.suggestionStatus} ${styles.suggestionError}`}>{locationSearchError}</p>}
          {locationSuggestions.length > 0 && (
            <ul className={styles.locationSuggestions}>
              {locationSuggestions.map((suggestion) => (
                ...
                >
                  <strong className={styles.suggestionName}>{suggestion.name}</strong>, {suggestion.address}
                </li>
              ))}
            </ul>
          )}
        ...
        <div className={styles.formGroup}>
          <label htmlFor="pickupDate" className={styles.label}>Preferred Pickup Date:</label>
          <input
            type="date"
            id="pickupDate"
            name="pickupDate"
        ...
        <div className={styles.formGroup}>
          <label htmlFor="returnDate" className={styles.label}>Preferred Return Date:</label>
          <input
            type="date"
            id="returnDate"
            name="returnDate"
        ...
        <div className={styles.formGroup}>
          <label htmlFor="images" className={styles.label}>Upload Image (Only 1):</label>
          <input
            type="file"
            id="images"
            name="images"
