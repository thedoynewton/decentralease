...
export default function LessorHome() {
  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      filtered = filtered.filter(post =>
        post.category_id.toString() === selectedCategory
      );
    }
    ...
    <Layout>
      <div className={styles.container}>
        <div className={styles.header}>
          <h1 className={styles.headerTitle}>My Rentals</h1>
          <button 
          ...
          <div className={styles.filterWrapper}>
            ...
            {showCategoryDropdown && (
              <div className={styles.categoryDropdown}>
                <div
                  className={`${styles.categoryItem} ${!selectedCategory ? styles.selected : ""}`}
                  onClick={() => {
                    setSelectedCategory("");
                ...
                {categories.map((category) => (
                  <div
                    ...
                    className={`${styles.categoryItem} ${
                      selectedCategory === category.id.toString() ? styles.selected : ""
                    }`}
                    onClick={() => {
              ...
              >
                ...
                <div className={styles.postInfo}>
                  ...
                  <div className={styles.dates}>
                    <span>Pickup: {new Date(post.pickup_date).toLocaleDateString()}</span>
                    <span>Return: {new Date(post.return_date).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
            ))
