...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq("user_id", userId);
      if (!error && Array.isArray(data)) {
        setListingImages(data.filter((listing) => listing.image_url));
        setFilteredImages(data.filter((listing) => listing.image_url));
      } else {
        setListingImages([]);
        setFilteredImages([]);
      }
  ...
  useEffect(() => {
    ...
    if (category) {
      filtered = filtered.filter(
        (listing) => listing.category_id?.toString() === category
      );
    }
    setFilteredImages(filtered);
  }, [search, category, listingImages]);

  // Get unique categories for filter dropdown
  const categories = Array.from(
    new Set(listingImages.map((listing) => listing.category_id).filter(Boolean))
  );

  const profile = {
    username: name,
    Listings: listingImages.length,
    listingImages: filteredImages,
  };
  ...
  const handleSwitchToLessee = async () => {
    if (address) {
      ...
      <div className={styles.igProfileContainer}>
        ...
        <div className={styles.igProfileHeader}>
          ...
          <div className={styles.igProfileInfo}>
            ...
            <div className={styles.mobileProfileNameListings}>
              <span className={styles.mobileListingsCount}>
                <strong>{profile.Listings}</strong> Listings
              </span>
            </div>
        ...
        <div className={styles.searchFilterRow}>
          <input
            type="text"
            placeholder="Search Listings title..."
            value={search}
        ...
        <div className={styles.igListingsGrid}>
          {profile.listingImages.length === 0 ? (
            <div
            ...
            profile.listingImages.map((img, idx) => (
              <div key={idx} className={styles.iglistingItem}>
                <img
                  src={img.image_url}
